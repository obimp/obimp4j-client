...
import io.github.obimp.packet.handle.cl.ContactListPacketHandler.Companion.OBIMP_BEX_CL_CLI_SRV_AUTH_REPLY
...
class ClientServerAuthReplyPacket(
    ...
    override var header: Header = OBIMPHeader(type = OBIMP_BEX_CL, subtype = OBIMP_BEX_CL_CLI_SRV_AUTH_REPLY)
    override var body: Body<WTLD> = OBIMPBody()

    init {
        body.content.add(WTLD(LongWord(0x0001), UTF8(accountName)))
        body.content.add(WTLD(LongWord(0x0002), Word(reply.code)))
        if (offlineFlag) {
            body.content.add(WTLD(LongWord(0x0003)))
        }
        offlineTime?.let { body.content.add(WTLD(LongWord(0x0004), DateTime(it))) }
