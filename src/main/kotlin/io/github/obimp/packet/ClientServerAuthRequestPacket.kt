...
import io.github.obimp.packet.handle.cl.ContactListPacketHandler.Companion.OBIMP_BEX_CL_CLI_SRV_AUTH_REQUEST
...
class ClientServerAuthRequestPacket(
    ...
    override var header: Header = OBIMPHeader(type = OBIMP_BEX_CL, subtype = OBIMP_BEX_CL_CLI_SRV_AUTH_REQUEST)
    override var body: Body<WTLD> = OBIMPBody()

    init {
        body.content.add(WTLD(LongWord(0x0001), UTF8(accountName)))
        body.content.add(WTLD(LongWord(0x0002), UTF8(reason)))
        if (offlineFlag) {
