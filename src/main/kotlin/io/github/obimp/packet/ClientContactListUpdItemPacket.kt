...
) : OBIMPPacket(OBIMPHeader(type = OBIMP_BEX_CL, subtype = OBIMP_BEX_CL_CLI_UPD_ITEM)) {
    init {
        addItem(WTLD(LongWord(0x0001), LongWord(itemID)))
        newParentGroupID?.let { addItem(WTLD(LongWord(0x0002), LongWord(newParentGroupID))) }
        item?.let {
